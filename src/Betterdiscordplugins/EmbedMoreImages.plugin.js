...
getSettingsPanel() {
	const panel = this.createHTML("<h2>General:</h2>", "", "overflow: auto; padding: 16px; background-color: #36393f; color: #ffffff; font-size: 16px;");
	const header = panel.firstChild;
	header.style = "margin-bottom: 16px; border-bottom: 1px solid #72767d;";

	const checkboxContainer = this.createHTML(`
	...
	const checkbox = checkboxContainer.firstChild;
...
const img = document.createElement("img");
	...
	this.preloadImage(img, href, progressValue => {
		const formattedProgressValue = parseFloat(progressValue).toFixed(2);
